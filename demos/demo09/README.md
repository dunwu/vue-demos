...
> 本文源自[vue 官方文档 - 表单输入绑定](https://cn.vuejs.org/v2/guide/forms.html)，并补充完善文中所有的 Demo。
...
HTML 原生的输入元素类型并不总能满足需求。幸好，Vue 的组件系统允许你创建具有完全自定义行为且可复用的输入组件。这些输入组件甚至可以和 `v-model` 一起使用！要了解更多，请参阅组件指南中的[自定义输入组件](https://cn.vuejs.org/v2/guide/components.html#在组件上使用-v-model)。
